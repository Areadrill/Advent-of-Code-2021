...
fn main() {
    ...
    let last_winning_draw_idx = earliest_wins.iter().max().unwrap();
    let last_winning_draw_number = draw_order[*last_winning_draw_idx as usize];
    let last_winning_board_idx = earliest_wins.iter().position(|it| it == last_winning_draw_idx).unwrap();
    let last_winning_board = &bingo_arrays[last_winning_board_idx];

    let undrawn_last_winning_board_numbers: u32 = last_winning_board.iter()
        .take(5)
        .flatten()
        .filter(|it| !draw_order[0..((*last_winning_draw_idx + 1) as usize)].contains(it))
        .map(|it| it.parse::<u32>().unwrap())
        .sum();

    println!("{}", undrawn_last_winning_board_numbers * last_winning_draw_number.parse::<u32>().unwrap());
}
